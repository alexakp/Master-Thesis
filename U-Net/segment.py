...
"""
Code taken from: https://github.com/AndreFagereng/polyp-GAN/blob/main/final_test_scripts/segmentation_script.py
...
#config.IMAGE_PATH = r"C:\Users\Alexander-PC-hjemme\Desktop\UiO\Master_Thesis\Pytorch-UNet\images\polyps-1"
#config.MASK_PATH = r"C:\Users\Alexander-PC-hjemme\Desktop\UiO\Master_Thesis\Pytorch-UNet\masks\polyps-1"

config.IMAGE_PATH = r"C:\Users\Alexander-PC-hjemme\Desktop\UiO\Master_Thesis\Pytorch-UNet\Images_data\big-data\images-boost-v4"
config.MASK_PATH = r"C:\Users\Alexander-PC-hjemme\Desktop\UiO\Master_Thesis\Pytorch-UNet\Images_data\big-data\masks-boost-v4"


#config.IMAGE_PATH = r"C:\Users\Alexander-PC-hjemme\Desktop\UiO\Master_Thesis\Pytorch-UNet\Images_data\big-data\images-clean"
#config.MASK_PATH = r"C:\Users\Alexander-PC-hjemme\Desktop\UiO\Master_Thesis\Pytorch-UNet\Images_data\big-data\masks-clean"

#config.IMAGE_PATH_VAL = r"C:\Users\Alexander-PC-hjemme\Desktop\UiO\Master_Thesis\Pytorch-UNet\images\polyps-3"
#config.MASK_PATH_VAL = r"C:\Users\Alexander-PC-hjemme\Desktop\UiO\Master_Thesis\Pytorch-UNet\masks\polyps-3"

config.IMAGE_PATH_VAL = r"C:\Users\Alexander-PC-hjemme\Desktop\UiO\Master_Thesis\Pytorch-UNet\Images_data\CVC-Clinic\images"
config.MASK_PATH_VAL = r"C:\Users\Alexander-PC-hjemme\Desktop\UiO\Master_Thesis\Pytorch-UNet\Images_data\CVC-Clinic\masks"

#config.IMAGE_PATH_VAL = r"C:\Users\Alexander-PC-hjemme\Desktop\UiO\Master_Thesis\Pytorch-UNet\transformed_dataset_128\images"
#config.MASK_PATH_VAL = r"C:\Users\Alexander-PC-hjemme\Desktop\UiO\Master_Thesis\Pytorch-UNet\transformed_dataset_128\masks"


config.IMAGE_PATH_TEST = r"C:\Users\Alexander-PC-hjemme\Desktop\UiO\Master_Thesis\Pytorch-UNet\Images_data\big-data\test-images"
...
class PolypDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, config, mode=''):
        ...
        if mode == 'train':
            self.image_paths = sorted(glob(config.IMAGE_PATH + '/*.png'))
            self.mask_paths  = sorted(glob(config.MASK_PATH + '/*.png'))
        elif mode == 'val': 
            self.image_paths = sorted(glob(config.IMAGE_PATH_VAL + '/*.png'))
            self.mask_paths  = sorted(glob(config.MASK_PATH_VAL + '/*.png'))
        elif mode == 'test':
            self.image_paths = sorted(glob(config.IMAGE_PATH_TEST + '/*.png'))
        else:
            raise ValueError
...
test_dataset = PolypDataset(config,'test')

#test_dataloader = PolypDataset(config, 'test')
...
def dont_use_this():
    string = "basev2"
    string = "base+800-best"
    string = "self-learn-2"



    ckpt = torch.load(f"self-learned-metric/{string}.ckpt")
    ...
    for i in range(1000):
        ...
        for image, pr_mask in zip(batch["image"], pr_masks):
            ...
            plt.subplot(1, 3, 1)
            plt.imsave(f"Images_data/big-data/images-boostv2-self-learn/img-{i}.png",image.numpy().transpose(1, 2, 0))
            plt.imshow(image.numpy().transpose(1, 2, 0))  # convert CHW -> HWC
            plt.title("Image")
            plt.axis("off")

            thresholded = numpy.where(pr_mask >= 0.5, 1, 0)
            plt.subplot(1, 3, 2)
            plt.imsave(f"Images_data/big-data/masks-boostv2-self-learn/img-{i}.png",thresholded.squeeze(),cmap='gray')
            plt.imshow(thresholded.squeeze(),interpolation='none',cmap='gray') # just squeeze classes dim, because we have only one class
            plt.title("Predicted")
            plt.axis("off")

            img = pr_mask.numpy().squeeze()
            img = img*5
            plt.subplot(1, 3, 3)
            #plt.imsave(f"paper-images2/{string}-heatmap-{i}.png",img,cmap='coolwarm', vmin=-100,vmax=100)
            plt.imshow(img,interpolation='none', cmap='coolwarm', vmin=-100,vmax=100) # just squeeze classes dim, because we have only one class
            plt.title("Heatmap")
            plt.axis("off")
...
def loadModel():
    string = "basev2"
    #string = "base+800-best"
    ckpt = torch.load(f"potenial_models/{string}.ckpt")
    model.load_state_dict(ckpt['state_dict'])
    ...
    for i in range(4):
        ...
        for image, gt_mask, pr_mask in zip(batch["image"], batch["mask"], pr_masks):
            ...
            plt.subplot(1, 4, 1)
            plt.imsave(f"paper-images2/input-image-{i+1}.png",image.numpy().transpose(1, 2, 0))
            plt.imshow(image.numpy().transpose(1, 2, 0))  # convert CHW -> HWC
            plt.title("Image")
            plt.axis("off")

            plt.subplot(1, 4, 2)
            plt.imsave(f"paper-images2/gt-{i+1}.png",gt_mask.numpy().squeeze(),cmap='gray')
            plt.imshow(gt_mask.numpy().squeeze(),interpolation='none',cmap='gray') # just squeeze classes dim, because we have only one class
            plt.title("Ground truth")
            plt.axis("off")

            thresholded = numpy.where(pr_mask >= 0.5, 1, 0)
            plt.subplot(1, 4, 3)
            plt.imsave(f"paper-images2/{string}-predicted-{i+1}.png",thresholded.squeeze(),cmap='gray')
            plt.imshow(thresholded.squeeze(),interpolation='none',cmap='gray') # just squeeze classes dim, because we have only one class
            plt.title("Predicted")
            plt.axis("off")
            ...
            plt.subplot(1, 4, 4)
            plt.imsave(f"paper-images2/{string}-heatmap-{i+1}.png",img,cmap='jet', vmin=0,vmax=1)
            plt.imshow(img, cmap='jet', vmin=-scale,vmax=scale) # just squeeze classes dim, because we have only one class
            plt.title("Heatmap")
            plt.axis("off")
...
# Either train_model or load_model with correct path to eval performance on images in val set. 
#loadModel()
